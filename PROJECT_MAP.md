...
| テスト | `npm test` | 型チェック（test/tsconfig.json）の後に node:test を実行。2026-09-11 時点で 150 ケース（test/routes 13 ファイル、test/lib 11 ファイル）。対象ファイルは package.json の test スクリプトに列挙しており、テストを追加したらここへも追記する。ts-node/register で動かすため tsx は不要 |
...
| MAX_UPSTREAM_BODY_BYTES | 任意 | 上流の 1 応答をメモリへ読み込む上限バイト数（既定: 536870912 = 512 MiB）。メタデータ補完と、メタデータ・検索・VPM インデックス・署名鍵の JSON 読み出しに適用される。npm 中継の tarball はストリームで返すためメモリには載らず、この上限は**キャッシュへ書き込んでよい大きさ**として働く（上限を超えるアーカイブは中継されるが保存されない）。VPM の zip 用上限とは別枠 |
...
- ただしキャッシュへの公開はいずれも一時ファイルへ書いてから rename する方式で行う。対象は metadata.json（`writeJsonAtomic`）、VPM の zip から変換した tgz（`convertZipBufferToTgz`）、npm 中継の tarball（`writeTarballCacheStream`。応答をクライアントへ流しながら同時に書き込む）の 3 経路で、これがキャッシュへ書き込む経路のすべてである。npm 中継のキャッシュ公開は応答の完了より後になりうる（同一の応答を分岐させているため）。したがってロックを取らない読み手（別プロセスを含む）が書きかけのファイルを読むことはない。失われうるのは同時更新のうち一方であり、壊れたファイルが残ることではない。
